...
if (context.Quote.GetCustomField('CF_TRANSACTION_TYPE').Value) == lc_trans_type : #Added by Dhurga
    cust_sign_date = context.Quote.GetCustomField('CF_Customer_Signed_Date').Value
    if cust_sign_date != "":
        cust_sign_date = UserPersonalizationHelper.CovertToDate(context.Quote.GetCustomField('CF_Customer_Signed_Date').Value)
        
       
        if getdate.todaysdate >= cust_sign_date:
            
            
       		context.Quote.GetCustomField('CF_Customer_Signed_Date').Value = cust_sign_date
        else:
            context.Quote.GetCustomField('CF_Customer_Signed_Date').Value =getdate.todaysdate#cxcpq-34880 end