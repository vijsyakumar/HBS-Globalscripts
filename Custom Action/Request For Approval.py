...
#-----------------------------------------------------------------------------
#Description:
#This script is used for maximum approval level
#-----------------------------------------------------------------------------
# Date            Name                Version        Comments(Changes done)
#-----------------------------------------------------------------------------
# 08/08/2022    Krishna Chaitanya          0             -Initial Version
# 10/20/2022    Ishika Bhattacharya        52             -Replaced Hardcodings
#                                                        -Incorporated Translation
# 10/31/2022    Aditi Sharma               66            -Fix for Maximum Recomm 
#                                                         Disc 'else'
# 11/01/2022    Aditi Sharma               67            -Replaced lc_op_type with lc_trans_type
#                                                         to remove dependency on multiple opp types
# 11/05/2022	Dhruv Bhatnagar			   68			 -Tranlation corrections
# 11/14/2022    Aditi Sharma               69            -To populate NEX feilds from CPQ
# 11/14/2022	Dhruv Bhatnagar			   70			 -Corrected Field name as LANGUAGE_KEY
# 11/22/2022    Aditi Sharma               76            -To calculate approval level based on
#                                                         USD quote sell price
# 11/22/2022    Aditi Sharma               77            -Correction to get proper rounding of USD price
# 11/28/2022    Aditi Sharma               78            -To eliminate parent write in from total price calculation
# 12/1/2022    Aditi Sharma                80            -Fixed rounding issues for prices
# 12/16/2022   Aditi Sharma                81            -Applied rounding till 2 decimals for discount variance, removed discVar method (was not being used in script)
# 12/22/2022   Aditi Sharma                83            -Added OR condition for Honeywell Labor as product type
# 12/23/2022   Aditi Sharma                84            -Added condition to avoid duplication of total sell price for mandatory charges parent
# 01/13/2023   Dhruv Bhatnagar			   87			 -CXCPQ-35217:replace pole/region criteria with country for fetching recommended
# 01/17/2023   Aditi Sharma                88            -Added changes to fetch approval for CSPA-RQ and non-CSPA-parts only
# 01/17/2023   Aditi Sharma                89            -Added missing variables lc_Discount, lc_cspa, lc_Y
# 01/18/2023   Sanket Mhapsekar                          -from line 164 below 5 lines added by sanket for ignore CXCPQ-33607
# 01/18/2023   Aditi Sharma                94            -Corrected the margin percent calculation for mandatory charges
...
if (context.Quote.GetCustomField('CF_TRANSACTION_TYPE').Value) == lc_trans_type : #added by Aditi 1st Nov 2022
    ...
    #lc_message = GM_TRANSLATIONS.GetText('000160', lv_LanguageKey, '', '', '', '', '') # Added by Ishika
    lc_max_discount_perc_recomm = GM_TRANSLATIONS.GetText('000137', lv_LanguageKey, '', '', '', '', '') # Added by Ishika
    lc_quote_list_price = GM_TRANSLATIONS.GetText('000132', lv_LanguageKey, '', '', '', '', '') # Added by Ishika
    lc_quote_sell_price = GM_TRANSLATIONS.GetText('000133', lv_LanguageKey, '', '', '', '', '') # Added by Ishika
    lc_margin_perc = GM_TRANSLATIONS.GetText('000134', lv_LanguageKey, '', '', '', '', '') # Added by Ishika
    lc_margin_amount = GM_TRANSLATIONS.GetText('000135', lv_LanguageKey, '', '', '', '', '') # Added by Ishika
    lc_actual_discount = GM_TRANSLATIONS.GetText('000162', lv_LanguageKey, '', '', '', '', '') # Added by Ishika
    ...
    lc_highest_approval_level = GM_TRANSLATIONS.GetText('000139', lv_LanguageKey, '', '', '', '', '') # Added by Ishika
    ...
    special_bmethods = []
    special_query = SqlHelper.GetList("SELECT * FROM CT_BUYING_METHODS WHERE SPECIAL_PRICING='Y' AND LANGUAGE_KEY = '"+str(lv_LanguageKey)+"'") #Corrected by Dhruv
    ...
    def getApprovalLevel(qbuying_mth, qsell_price, qdisc_var, qmargin): #modified by Aditi 17 Jan
        # qmm = qmargin
        maxApproval = ''
    ...
    def quote_currency_conversion(USD_value,ToCurrencyCode ):
        Conversion_Rate = SqlHelper.GetFirst("SELECT * FROM CT_EXCHANGE_RATE WHERE FROM_CURRENCY= '{}' and TO_CURRENCY= '{}' ORDER BY DATE DESC ".format(
    ...
    if funcApprovalRecords:
        for record in funcApprovalRecords:
            #Trace.Write("==Request for Approval, Approval_Status=="+str(record.Approval_Status))
            #Trace.Write("=== Request for Approval record.Required.lower()===="+str(record.Required.lower()))
            #if(record.Required.lower() == "true" and record.Approval_Status != "Approved"):   #Commented by Ishika
            #if (record.Required.lower() == lc_true and record.Approval_Status != lc_approved):  #Added by Ishika
            ...
            if (record.Required.lower() == lc_true and record.Approval_Status != lc_approved):#Added by Ishika below 5 lines added by sanket for ignore CXCPQ-33607
                if record.Type_of_Approval.lower()=="honeywell t & c" or record.Type_of_Approval.lower()=="opportunity credit check" or record.Type_of_Approval.lower()=="payment terms" :
                    approvalStatusFlag=approvalStatusFlag
                else:
                    approvalStatusFlag = lc_False
                    break
    if approvalStatusFlag == lc_False:
        #throw error message
        # Log.Info("==== Quote is not relevant/eligible for Functional Approval ===")
        # msg = "Functional Approval is in Progress" #Commented by Ishika
        ...
        CurrencyCode = context.Quote.GetCustomField('CF_Quote_Currency').Value #context.Quote.SelectedMarket.CurrencyCode #modified by Aditi

        summaryInfoTable = SqlHelper.GetList("SELECT * FROM CT_FinancialSummary_Info where LanguageKey = '"+str(lv_LanguageKey)+"' ")
        ...
        for qitem in context.Quote.GetAllItems():
            if qitem.DiscountAmount != None and qitem.NetPrice != None and qitem["QI_Recommended_Unit_Sell_Price"] != None and qitem["QI_MARGIN_PERCENTAGE"] != None and qitem["QI_Recommended_Sell_Price"] != None and qitem['QI_Product_Type'] != 'Write-In' and qitem['QI_Product_Type'] != '' and qitem.IsOptional == False and qitem.ProductSystemId != 'RQ_Mandatory_Charges_cpq': #Modified by Aditi 23rd Dec
                ...
                counter += 1
        #####Added by Aditi for NEX fields mapping 14 Nov 2022
        if totQuoteValue:
        ...
        if counter!=0 and totalSellPrice!=0: #modified by Aditi 18 Jan 2023
            ...
            context.Quote.GetCustomField('CF_Gross_Margin').Value = round((totMarginPer), 2) #Added by Aditi for NEX field mapping 14 Nov 2022
        ...
        for entry in summaryInfoTable:
            ...
            if entry.Summary_Info == lc_quote_list_price:   #Added by Ishika
                ...
                row["USD_Currency"] = lc_USD + " " + str(round(quote_currency_conversion(totQuoteValue, CurrencyCode),2)) #Added by Ishika #modified by Aditi
                row["Quote_Currency"] = CurrencyCode + " " + str(round(totQuoteValue,2)) #modified by Aditi
            ...
            if entry.Summary_Info == lc_quote_sell_price:  #Added by Ishika
                ...
                row["USD_Currency"] = lc_USD + " " + str(round(quote_currency_conversion(totalSellPrice, CurrencyCode),2))  #Added by ishika #modified by Aditi
                row["Quote_Currency"] = CurrencyCode + " " + str(round(totalSellPrice,2)) #modified by Aditi
            ...
            #if entry.Summary_Info == "Margin Amount":  #Commented by Ishika
            ...
            #if entry.Summary_Info == "Actual Discount":  #Commented by Ishika
            if entry.Summary_Info == lc_actual_discount: #Added by Ishika
            ...
            if country:								#Inserted By Dhruv CXCPQ-35217
                if solnfamily:						#Inserted By Dhruv CXCPQ-35217
                    if vertical:
                        ...
                        '''primaryVerQry = SqlHelper.GetList("SELECT * FROM CT_Primary_Recommendations WHERE Pole='{}' AND Region='{}' AND SolutionFamily='{}' AND Vertical='{}' AND LanguageKey='{}'".format(pole,region,solnfamily,vertical,lv_LanguageKey))#Modified by Dhruv''' #Commented By Dhruv CXCPQ-35217
                        ...
                        if primaryVerQry:
                            ...
                            for entry in primaryVerQry:
                                ...
                                if fppcontent>=entry.FPPContentLowRange and fppcontent<=entry.FPPContentHighRange and labcontent>=entry.LaborContentLowRange and labcontent<=entry.LaborContentHighRange:
                                    ...
                                    for fin_row in finSummTable.Rows:
                                        ...
                                        if fin_row['Summary'] == lc_max_discount_perc_recomm:  #Added by Ishika
                            ...
                            if not matched:
                                '''secondaryQry = SqlHelper.GetList("SELECT * FROM CT_Secondary_Recommendations WHERE SC_Pole='{}' AND LanguageKey='{}'".format(pole,lv_LanguageKey))'''#Commented By Dhruv CXCPQ-35217
                                secondaryQry = SqlHelper.GetList("SELECT * FROM CT_Secondary_Recommendations WHERE Country='{}' AND LanguageKey='{}'".format(country,lv_LanguageKey)) #Inserted by Dhruv CXCPQ-35217
                            ...
                            if secondaryQry:
                                for entry in secondaryQry:
                                    ...
                                    if fppcontent>=entry.SC_FPPContentLowRange and fppcontent<=entry.SC_FPPContentHighRange:
                                        ...
                                        for fin_row in finSummTable.Rows:
                                            #if fin_row['Summary']=='Maximum Discount % Recommended':
                                            if fin_row['Summary'] == lc_max_discount_perc_recomm:  #Added by Ishika
                                                #Trace.Write("Sec Found "+str(fin_row['Summary'])+str(entry.SC_NewDiscPercentRecomm))
                                                fin_row['Quote_Currency']=str(entry.SC_NewDiscPercentRecomm)+"%"
                                                #Trace.Write("MaxDiscount: "+str(fin_row['Quote_Currency']))
                        ...
                        '''primaryQry = SqlHelper.GetList("SELECT * FROM CT_Primary_Recommendations WHERE Pole='{}' AND Region='{}' AND SolutionFamily='{}' AND LanguageKey='{}'".format(pole,region,solnfamily,lv_LanguageKey))'''#Commented By Dhruv CXCPQ-35217
                        primaryQry = SqlHelper.GetList("SELECT * FROM CT_Primary_Recommendations WHERE Country='{}' AND SolutionFamily='{}' AND LanguageKey='{}'".format(country,solnfamily,lv_LanguageKey)) #Inserted by Dhruv CXCPQ-35217
        ...
        for fs_row1 in finSummTable.Rows:
            ...
            if fs_row1['Summary'] == lc_max_discount_perc_recomm:   #Added by Ishika
            ...
            # if fs_row1['Summary'] == 'Actual Discount':   #Commented by ishika
            if fs_row1['Summary'] == lc_actual_discount: #Added by Ishika
                if fs_row1['Quote_Currency']:
            ...
            if fs_row1['Summary'] == lc_discount_variance_perc:   #Added by Ishika
                if maxRecDisc!=None and actDisc!=None:
                    fs_row1['Quote_Currency'] = round(actDisc - maxRecDisc,2)
                    fs_row1['Quote_Currency'] = str(fs_row1['Quote_Currency'])+"%"
                    context.Quote.GetCustomField('CF_Discount_Variance').Value = round(actDisc - maxRecDisc, 2)   #Added by Ishika 16dec #Added by Aditi for NEX field maping 14 Nov 2022
        ...
        for fs_row in finSummTable.Rows:
            #Log.Info('FS_Quote Curr -->'+str(fs_row['Quote_Currency'])+ 'FS_Disc Var -->'+str(lc_discount_variance_perc))
            #Log.Info('FS_USD Curr -->'+str(fs_row['USD_Currency']) )
            ...
            if fs_row['Summary'] == lc_discount_variance_perc:  #Added by Ishika
                if fs_row['Quote_Currency']:
                    ...
                    Log.Info('Quote Currency -->'+str(fs_row['Quote_Currency']) )
        ...
        for fs_row in finSummTable.Rows:
            ...
            if fs_row['Summary'] == lc_highest_approval_level:   #Added by Ishika
                fs_row['Quote_Currency'] = maxApproval
        '''
        req_approvalLevel = context.Quote.GetCustomField("CF_APPROVAL_LEVEL").Value
        if itemCount > 0 and not maxApproval.startswith('Auto'):
            #status = context.Quote.StatusName
            #Trace.Write('statusss:{}'.format(status))
            message1 = 'Required Approval level: ' +str(maxApproval)
        ...
        '''
        #Added by Aruditi for status
        # context.Quote.ChangeStatus('Ready For Approval')  #Commented by Ishika
        context.Quote.ChangeStatus(lc_status_ready_for_approval)   #Added by Ishika
        #Log.Info("=== Request for Approval else logic end====")